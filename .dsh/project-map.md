...
## 模块划分
- `main/`（主进程，40+ 模块）：main.js 总装（IPC/RPC/菜单/托盘/启动/自动接续）；harness.js 服务控制；web-patch.js 生成 web.patch.yml 注入客户端插件与 MCP；mcp-server.mjs 内置 MCP 工具（stdio）；desktop-rpc.js 桌面端 HTTP RPC 服务；github.js GitHub 设备码登录/仓库/搜索（纯 Node 可测）
...
- main/github.js：设备码登录（deviceFlowStart/Poll）、token 存取（.github-auth.json）、whoami/createRepo/searchCode/status、suggestRepoName
...
- main/main.js：startRpc() 注册 RPC（github* + git*）；registerDesktopFeatureIpc() 注册 github IPC；auto-resume 接续
- packages/settings-update/client.js：SchedPanel 按 parentSession 过滤；EnvPanel 含 Git/项目地图/编辑占用/GitHub 组（5s 高频 + 推送）；GithubSection 设置分区（登录码轮询/一键关联远程/断开）
- renderer/git-diff.html：Git 变更窗（分支/提交/回滚/复制 + 占用警告横幅）
- scripts/test-github.js：github.js 单测 15 项（注入式 transport）
