    ...
    function EnvPanel(props) {
      ...
      return h('div', { style: panelBody, 'data-dsh-desktop-env-panel': 'true' },
        ...
        h('div', { style: { display: 'flex', flexDirection: 'column' } },
          ...
          envRow('项目', ((state && state.ui && state.ui.cwd) || (state && state.workspace) || '—'), 'ws'),
          envRow('harness', (state && state.installed) ? 'v' + state.installed : '—', 'ver'),
          envRow('会话', currentSessionId
            ? ((state && state.ui && state.ui.sessionId === currentSessionId && state.ui.title) || String(currentSessionId).slice(0, 12))
            : '—', 'sid'),
